...
Flutter 校车管理系统，覆盖普通用户端、司机端和后台管理端。

## 已完成

- 工程结构：Flutter 3.x 工程与模块化 `lib/` 目录。
- 状态管理：使用 `flutter_riverpod` 管理入口模式、登录态和业务状态。
- 路由：使用 `go_router` 配置登录鉴权和三端首页。
- 三端入口：通过 `APP_MODE` 环境变量初始化普通用户端、司机端和后台管理端。
- 登录页：内置普通用户、司机、管理员三组演示账号。
- 用户端：可预约车次列表、选座确认、生成预约订单、我的预约筛选、取消预约扣信用分。
- 司机端：今日派车任务、乘客名单/上车点、日/周/月统计看板、实时位置状态共享。
- 后台端：车辆管理、司机管理、实时位置监控、预约需求汇总、人工调度、智能辅助调度、客户 ETA 和历史数据分析。
- 支付与通知：支付流程、信用等级、失信记录、通知中心和 UI 收口。
...
账号角色必须与当前入口一致。点击登录页的演示账号会自动切换入口并填入账号。
...
- 系统会立即生成一次位置上报，并每 `5` 秒追加一条经纬度和速度记录。
- 点击 `上报一次` 可手动追加位置记录，便于演示位置变化。
...
- 实时位置监控使用 Flutter 组件绘制地图标记，点击车辆标记可查看车次、司机、速度和坐标，并展示预约客户距离和预计到达时间。
- 位置监控每 `5` 秒自动模拟更新一次，也可点击 `刷新位置` 手动更新。

## 调度与分析说明

- `智能生成方案` 会按待调度人数降序匹配空闲且已绑定司机的车辆。
...
- 最终报告：`docs/08_最终项目报告.pdf`
- 项目计划：`docs/01_项目计划.docx`
- 需求文档：`docs/02_项目需求.pdf`
- 设计文档：`docs/03_项目设计.doc`
- 源码说明：`docs/04_项目源码说明.pdf`
- 测试计划：`docs/05_1_测试计划_司机端.docx`
- 测试用例：`docs/05_2_测试用例_司机端.docx`
- 司机端测试报告：`docs/05_3_测试报告_司机端.docx`
- 用户端测试报告：`docs/05_4_测试报告_用户端.docx`
- 后台端测试报告：`docs/05_5_测试报告_后台端.docx`
- 配置管理：`docs/06_配置管理.docx`
- 工具标识：`docs/07_课程工具使用标识.pdf`
- UML 说明：`docs/09_StarUML设计过程.pdf`
...
- `docs/00_项目报告总览.pdf`：报告索引和完成情况总览。
- `docs/08_最终项目报告.pdf`：期末提交总入口，串联计划、需求、设计、源码、测试、配置管理和工具标识。
- `docs/01_项目计划.docx`：项目计划、WBS、里程碑和甘特图。
- `docs/02_项目需求.pdf`：需求说明、业务规则和用例。
- `docs/03_项目设计.doc`：架构设计、模块设计、数据模型和 UML。
- `docs/04_项目源码说明.pdf`：源码结构、运行方式和功能映射。
- `docs/05_1_测试计划_司机端.docx`：司机端测试计划。
- `docs/05_2_测试用例_司机端.docx`：司机端测试用例。
- `docs/05_3_测试报告_司机端.docx`：司机端测试报告。
- `docs/05_4_测试报告_用户端.docx`：用户端测试报告。
- `docs/05_5_测试报告_后台端.docx`：后台端测试报告。
- `docs/06_配置管理.docx`：Git、依赖、基线和交付配置管理。
- `docs/07_课程工具使用标识.pdf`：课程工具使用痕迹汇总。

## 支付、信用和通知说明

- 用户端待支付预约可点击 `立即支付`，选择 `微信支付` 或 `支付宝`。
- 支付流程会先进入 `支付中`，约 `500ms` 后变为 `已支付`，并写入支付流水。
- 预约创建、支付成功、取消扣分都会写入通知中心。
