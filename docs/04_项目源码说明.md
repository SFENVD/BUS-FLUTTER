...
`[课程工具标识: Flutter/Dart + Android Studio/VS Code + Git]` 本部分对应“项目源码”要求。项目源码使用 Flutter 和 Dart 开发，依赖通过 `pubspec.yaml` 管理，版本通过 Git 管理，自动化测试位于 `test/` 目录。
...
| 文件 | 说明 |
| --- | --- |
| `lib/features/auth/presentation/login_page.dart` | 登录界面和演示账号快捷填充 |
| `lib/features/auth/data/` | 账号、登录校验和角色匹配校验 |
...
| `lib/features/driver/tasks/data/` | 司机任务和统计数据 |
...
| `lib/features/admin/data/` | 车辆、司机、位置、需求和分析数据 |
...
| 智能辅助调度 | `generateAiDispatchPlans` |
| 调度确认 | `confirmDispatchPlan` |
| 实时位置监控 | `_TrackingPanel`、`_VehicleTrackingDetail` |
...
| 车辆位置监控 | `VehicleLocationModel`、`_TrackingPanel` |
| 客户距离和 ETA | `_PassengerEtaPanel` |
| 历史数据分析 | `AdminAnalyticsSnapshot`、`_AnalyticsPanel` |

## 测试入口

自动化测试文件位于 `test/` 目录，覆盖普通用户、司机和后台管理端关键流程，以及角色入口、重复预约、不可取消、车辆删除、司机绑定和调度失败等负向业务规则。运行命令：
