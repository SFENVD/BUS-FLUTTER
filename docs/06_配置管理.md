...
| 业务规则测试基线 | `test/business_rule_test.dart` | 关键负向业务规则自动化测试通过 |
...
| `dart format --set-exit-if-changed lib test` | 提交前检查源码和测试格式是否已规范化 |
| `flutter analyze` | 执行 Flutter 静态分析 |
| `flutter test` | 执行自动化测试 |
| `flutter build web --release --dart-define=APP_MODE=admin` | 构建后台管理端 Web release 包 |
