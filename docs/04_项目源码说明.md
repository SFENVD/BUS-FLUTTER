...
`[课程工具标识: Flutter/Dart + Android Studio/VS Code + Git]` 本部分对应“项目源码”要求。项目源码使用 Flutter 和 Dart 开发，依赖通过 `pubspec.yaml` 管理，版本通过 Git 管理，自动化测试位于 `test/widget_test.dart`。
...
自动化测试文件为 `test/widget_test.dart`，覆盖普通用户、司机和后台管理端关键流程。运行命令：
