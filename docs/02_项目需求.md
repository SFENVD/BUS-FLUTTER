...
- **数据策略**：使用内置演示数据仓库支撑课程演示、自动化测试和业务流程闭环
- **交付策略**：以三端完整流程、UML 建模、测试文档和配置管理文档作为课程交付依据
...
| **司机** | Driver | 校车驾驶员 | 查看派车任务、查看乘客名单和上车点、发车后共享实时位置状态、查看任务统计 |
...
| UC-15 | 智能辅助调度 | 管理员 | 高 | 在调度面板选择智能推荐模式 |
...
| `<<extend>>` | 智能辅助调度 | 人工调度 | 智能推荐是人工调度的辅助扩展 |
...
| BRQ-03 | 支持调度决策 | 后台进行人工调度和智能辅助调度，降低空车/超载风险 | UC-14 ~ UC-15 |
| BRQ-04 | 掌握运行状态 | 后台查看车辆司机实时位置状态、车次状态、客户 ETA | UC-16 ~ UC-17 |
...
| 4 | DriverTaskController | DriverTaskController | 生成首条位置记录 |
...
| 13 | PaymentService | PaymentService | 支付确认处理 |
...
| `PassengerRepository` | 数据仓库 | 乘客端业务数据 |
| `DriverRepository` | 数据仓库 | 司机端业务数据 |
| `AdminRepository` | 数据仓库 | 后台端业务数据 |
| `AuthRepository` | 数据仓库 | 认证与用户数据 |
| `BackendBootstrap` | 数据引导 | 业务数据初始化 |

**依赖关系（19条）**：所有 Repository 依赖 CoreModels；Provider 通过 Repository 读取和更新业务数据；BackendBootstrap 管理业务数据初始化。
...
| NFR-04 | 可扩展性 | 数据仓库层与页面层解耦，便于维护和扩展 | 设计审查 |
| NFR-05 | 响应式 | 使用 LayoutBuilder 适配不同屏幕 | 多尺寸测试 |
| NFR-06 | 性能 | 位置上报 5 秒，支付确认 ≤ 500ms | 代码审查 |
...
| RUL-11 | 智能调度只选空闲且已绑定司机的车辆 | 智能调度 | 返回失败提示 |
...
| FR-05 | 支付车费 | 支付流程 | `payment_model.dart` | TC-04 |
...
| FR-12 | 人工和智能调度 | 调度模块 | `dispatch_model.dart` | TC-12, TC-13, TC-23, TC-24 |
